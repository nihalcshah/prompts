...
export default function TagsClient({ tags }: TagsClientProps) {
  ...
  const handleDeleteSelected = async () => {
    ...
    const confirmed = confirm(
      `Are you sure you want to delete ${selectedTags.length} tags? This will remove the tags from all associated prompts.`
    );
    if (!confirmed) return;
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        router.push(
                          `/admin/tags/${encodeURIComponent(tag.name)}/edit`
                        );
                      }}
